...
namespace BLL.AdminService
{
    public class AdminService : commonclass, IAdminService
    {
        ...
        List<ProfileDtos> IAdminService.GetUsers(UserType userType)
        {
            try
            {
                var users = db.Users.Where(p => p.Type == userType).Where(p => p.Status != UserStatus.Deleted).OrderByDescending(p => p.CreatedAt).AsEnumerable().Select(n => new ProfileDtos {
        ...
        List<ProfileDtos> IAdminService.GetStaffUsers(UserType userType,string userid)
        {
            try
            {
                var staff = db.Users.Find(userid);
                var branch = db.Branches.Find(staff.BranchId);
                var users = db.Users.Where(p => p.Type == userType && p.BranchId == branch.Id).Where(p=>p.Status != UserStatus.Deleted).OrderByDescending(p => p.CreatedAt).AsEnumerable().Select(n => new ProfileDtos
                {
                    CreatedAt = n.CreatedAt.ToString("dd-MMM-yyyy hh:mm:ss tt"),
                    Email = n.Email,
                    FullName = n.FullName,
        ...
        async Task<ResponseDto> IAdminService.Login(LoginViewModel LoginUser)
        {
            try
            {
                ...
                if(RequestedUser.Status != UserStatus.Active)
                {
                    return JsonResponse2(400, "You cannot login with this user", null);
                }
        ...
        public List<ProductDtos> GetProducts()
        {
            try
            {
                var entities = db.Products.Where(p => p.Status != EntityStatus.Deleted).Select(n => new ProductDtos
                {
        ...
        public List<ProductDtos> GetStaffProducts(string userId)
        {
            try
            {
                var staff = db.Users.Find(userId);
                var branch = db.Branches.Find(staff.BranchId);
                var entities = db.Products.Where(p => p.Status != EntityStatus.Deleted).Where(p=>p.CategoriesObject.BranchId == branch.Id).Select(n => new ProductDtos
                {
                    CreatedAt = n.CreatedAt.ToShortDateString(),
                    Name = n.Name,
                    Status = n.Status,
        ...
        public List<IngredientListVms> GetIngredient()
        {
            try
            {
                var entities = db.Ingredients.Where(p=>p.Status != EntityStatus.Deleted).Select(n => new IngredientListVms
                {
                    CreatedAt = n.CreatedAt.ToShortDateString(),
                    Name = n.Name,
                    Status = n.Status,
        ...
        {
            var entities = db.Orders.Where(p=>p.UserId == userId && p.Status != OrderStatus.Delivered).OrderByDescending(p => p.CreatedAt).Select(n => new GetOrderdetails
            {
                CreatedAt = n.CreatedAt.ToShortDateString(),
                OrderId = n.OrderId,
                UpdatedAt = n.UpdatedAt.Value,
        ...
        {
            var entities = db.Orders.Where(p => p.UserId == userId).Where(p=> p.Status == OrderStatus.Delivered || p.Status == OrderStatus.Cancelled).OrderByDescending(p=>p.CreatedAt).Select(n => new GetOrderdetails
            {
                CreatedAt = n.CreatedAt.ToShortDateString(),
                OrderId = n.OrderId,
                UpdatedAt = n.UpdatedAt.Value,
